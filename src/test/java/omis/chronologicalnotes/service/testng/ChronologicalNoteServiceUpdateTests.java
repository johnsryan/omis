...
extends AbstractHibernateTransactionalTestNGSpringContextTests {
	 ...
	 * 
	 * @throws ChronologicalNoteExistsException Thrown if a chronological note already exists with the specified
	 * update date and update narrative.
	 */
	@Test
	public void testUpdate() throws ChronologicalNoteExistsException {
		...
		ChronologicalNote note = this.chronologicalNoteDelegate.create(date, offender, narrative);
		
		Date updatedDate = this.parseDateText("02/02/2018");
		String updatedNarrative = new String("This is the updated narrative of the test chronological note");
		
		//Action
		note = this.chronologicalNoteService.update(note, updatedDate, updatedNarrative);
		
		//Assertions
				PropertyValueAsserter.create()
					.addExpectedValue(DATE_PROPERTY_VALUE_NAME, updatedDate)
					.addExpectedValue(OFFENDER_PROPERTY_VALUE_NAME, offender)
					.addExpectedValue(NARRATIVE_PROPERTY_VALUE_NAME, updatedNarrative)
	...
	public void testChronologicalNoteExistsException()
		...
		Date updatedDate = this.parseDateText("02/02/2018");
		String updatedNarrative = new String("This is the updated narrative of the test chronological note");
		ChronologicalNote noteToUpdate = this.chronologicalNoteDelegate.create(updatedDate, offender, updatedNarrative);
		//Action
		this.chronologicalNoteService.update(noteToUpdate, date, narrative);
	}
